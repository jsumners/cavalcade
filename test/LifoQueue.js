...
const lifoQueueFactory = require(path.join(__dirname, '..', 'lib', 'LifoQueue'))

suite('LifoQueue', function () {
  suite('#ctor(size, initial)', function () {
    test('creates queues with a default size of 16', function (done) {
      const q = lifoQueueFactory()
      expect(q.size).to.equal(16)
      done()
    })

    test('creates queues with a specified size', function (done) {
      const q = lifoQueueFactory(2)
      expect(q.size).to.equal(2)
      done()
    })

    test('creates queues with a specified size and initial values', function (done) {
      const q = lifoQueueFactory(2, [1, 2])
      expect(q.size).to.equal(2)
    ...
    test('will not create a queue with an invalid size', function (done) {
      expect(lifoQueueFactory.bind(null, 1.2)).to.throw(Error)
      done()
    })

    test('will not create a queue with an invalid initial value', function (done) {
      expect(lifoQueueFactory.bind(null, 1, 1)).to.throw(Error)
      done()
    })
  })
  ...
  suite('#push(item)', function () {
    test('adds to the beginning of the queue', function (done) {
      let q = lifoQueueFactory(2)
      q.push(1)
  ...
  suite('#values()', function () {
    test('does not modify the queue', function (done) {
      const q = lifoQueueFactory(4, [1, 2, 3, 4])
